...
recs = 12; % 1-12 are 2019 recordings (eeeeeecx), 13-20 are 2020 recordings (cceeee....)
...
xlsFileName = 'testi_inter_ch_2019_2.xlsx';
...
for reg = 1:recs
    ...
    for i = 1:numel(header)
        ...
        writematrix(string(header{i}), xlsFileName, 'Sheet', reg, 'Range', solu);
    end
    ...
    for rCh = startCh:startCh+chCount-1
        for cCh = startCh:startCh+chCount-1
            ...
            writematrix(string(channelLabels{reg}{rCh}+"/"+channelLabels{reg}{cCh}), xlsFileName, 'Sheet', reg, 'Range', solu);
            for i = 1:6
                solu = strcat(char(65 + i), num2str(row));
                writematrix(squeeze(p(reg, rCh, cCh, i)), xlsFileName, 'Sheet', reg, 'Range', solu);
            end
            row = row + 1;
        end
    end
        ...
        for cCh = startCh:startCh+chCount-1
            ...
            for n = 1:6
                ...
                if (squeeze(p(reg, rCh, cCh, n)) < 0.01) && (squeeze(h(reg, rCh, cCh, n)) == 1)
                    WB.Worksheets.Item(reg).Range(solu).Interior.ColorIndex = 33;
                elseif (squeeze(p(reg, rCh, cCh, n)) < 0.01) && (squeeze(h(reg, rCh, cCh, n)) == -1)
                    WB.Worksheets.Item(reg).Range(solu).Interior.ColorIndex = 45;
                end
            end
        end
    end
